...
export default async function Today() {
  ...
  return (
    <div className="mx-auto max-w-[1080px] px-6 pb-20 pt-10 lg:px-10">
      ...
      <div className="mt-9 grid gap-10 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
        <section>
          ...
          <div className="space-y-2">
            ...
            }) : <div className="rounded-lg border border-border bg-bg-card px-4 py-8"><p className="text-[15px] font-medium text-ink">Your first thought starts here.</p><p className="mt-1 text-[13px] text-ink-faint">Capture freely. A matching hashtag handles the filing.</p></div>}
          </div>
        </section>
        ...
        <div className="space-y-11">
          <section>
            ...
            <Link href="/inbox" className="flex items-center gap-3 rounded-lg border border-border bg-bg-card px-3.5 py-3.5 hover:border-ink-ghost"><span className="grid h-8 min-w-8 place-items-center rounded-full bg-amber-tint font-mono text-[11px] text-amber-ink">{inboxCount ?? 0}</span><span><span className="block text-[13px] font-medium text-ink">Inbox to triage</span><span className="text-[12px] text-ink-faint">Unmatched and untagged thoughts</span></span></Link>
            <div className="mt-2 overflow-hidden rounded-lg border border-border bg-bg-card px-3">
              {openTasks?.length ? openTasks.map((task) => <div key={task.id} className="flex items-center gap-3 border-b border-border-soft py-3 text-[13px] last:border-b-0"><span className="h-3.5 w-3.5 rounded-full border border-ink-ghost"/><span className="min-w-0 flex-1 truncate text-ink-body">{task.title}</span><span className="shrink-0 text-[11px] text-ink-faint">{dueLabel(task.due_date)}</span></div>) : <p className="py-3 text-[12px] text-ink-faint">No open tasks yet.</p>}
            </div>
          </section>
